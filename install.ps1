...
$isCodeServer = $false
if ($env:CODE_SERVER -or (Get-Command "code-server" -ErrorAction SilentlyContinue)) {
    $isCodeServer = $true
    Write-Host "code-server environment detected" -ForegroundColor Green
} elseif (Test-Path "$env:APPDATA\code-server") {
    $isCodeServer = $true
    Write-Host "code-server environment detected" -ForegroundColor Green
}

# Check if VS Code is installed (for desktop VS Code)
if (-not $isCodeServer) {
...
if ($isCodeServer) {
    Write-Host "code-server detected: Custom UI Style extension needs manual installation" -ForegroundColor Yellow
    Write-Host "   Please install it manually from the Extensions marketplace in code-server"
    Write-Host "   Search for: Custom UI Style (by subframe7536)"
} else {
    try {
...
if ($isCodeServer) {
    $settingsDir = "$env:APPDATA\code-server\User"
} else {
...
Write-Host ""
Write-Host "Step 5: Finalizing installation..."
...
if (-not (Test-Path $firstRunFile)) {
    ...
    if (-not $isCodeServer) {
        ...
        Write-Host "   - For code-server: Reload the browser page to apply changes"
        Write-Host "   - Enable Custom UI Style from the Command Palette after reload"
    }
    Write-Host ""
...
if ($isCodeServer) {
    Write-Host "   Please reload your browser page to apply changes."
    Write-Host "   Then enable Custom UI Style from the Command Palette (Ctrl+Shift+P)"
} else {
